...
document.querySelectorAll(".max-btn").forEach(btn => {
  ...
  btn.addEventListener("click", () => {
    ...
    win.classList.add("size-transition");
    ...
    // Remove the transition class once the animation finishes, so it
    // doesn't interfere with (slow down) subsequent dragging.
    setTimeout(() => {
      win.classList.remove("size-transition");
    }, 250);
...
// =============================
// DRAG WINDOWS (shared mouse + touch handler, translate3d + RAF throttled)
// =============================
...
function makeWindowDraggable(win) {
  ...
  function applyTransformRAF() {
    ...
    rafId = null;
    win.style.transform = `translate3d(${pendingDx}px, ${pendingDy}px, 0)`;
  ...
  function dragStart(clientX, clientY) {
    ...
    // offsetLeft/offsetTop reflect current computed position
  ...
  function dragMove(clientX, clientY) {
    ...
    pendingDx = clientX - startX;
    pendingDy = clientY - startY;

    if (rafId === null) {
      rafId = requestAnimationFrame(applyTransformRAF);
    }
  ...
  function dragEnd() {
    ...
    // Commit the final position into left/top, then clear the transform,
    // so maximize/restore/minimize (which read win.style.left/top) keep working.
    win.style.left = baseLeft + pendingDx + "px";
    win.style.top = baseTop + pendingDy + "px";
    win.style.transform = "";
...
// =============================
// WINDOW RESIZING (drag from edges/corners)
// =============================
...
windows.forEach(makeWindowResizable);
...
// =============================
// SEARCH BAR (working search: filters apps, Enter opens first match)
// =============================
...
const startSearchEmpty = document.getElementById("start-search-empty");
...
if (searchIcon) {
  ...
  searchIcon.addEventListener("click", () => {
    ...
    if (wrapper.classList.contains("active") && taskbarSearchInput) {
      taskbarSearchInput.focus();
    }
...
function getSearchableApps() {
  return Array.from(document.querySelectorAll(".start-grid .icon"));
}
...
function filterApps(query) {
  ...
  apps.forEach(app => {
    ...
    const label = app.querySelector("span")?.textContent?.toLowerCase() || "";
  ...
  if (startSearchEmpty) {
    startSearchEmpty.classList.toggle("hidden", !(q !== "" && visibleCount === 0));
  }
...
function openFirstMatch(query) {

  const q = query.trim().toLowerCase();

  if (q === "") return;

  const match = getSearchableApps().find(app => {
    const label = app.querySelector("span")?.textContent?.toLowerCase() || "";
...
// Taskbar search: opens/filters the start menu as you type
if (taskbarSearchInput) {

  taskbarSearchInput.addEventListener("input", () => {

    const query = taskbarSearchInput.value;
    ...
    if (startSearchInput) startSearchInput.value = query;
  ...
  taskbarSearchInput.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Enter") {
      ...
      openFirstMatch(taskbarSearchInput.value);
  ...
  // Prevent the document click-to-close-start-menu handler from firing
  // when the click originated inside the taskbar search box.
  taskbarSearchInput.addEventListener("click", (e) => e.stopPropagation());

}

// Start-menu's own search box
if (startSearchInput) {

  startSearchInput.addEventListener("input", () => {

    const query = startSearchInput.value;

    filterApps(query);

    if (taskbarSearchInput) taskbarSearchInput.value = query;
  ...
  startSearchInput.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Enter") {
      ...
      openFirstMatch(startSearchInput.value);
  ...
  startSearchInput.addEventListener("click", (e) => e.stopPropagation());

}

// Reset the filter whenever the start menu is closed, so it's fresh next open
document.addEventListener("click", () => {
  if (startSearchInput) startSearchInput.value = "";
  if (taskbarSearchInput) taskbarSearchInput.value = "";
  resetAppFilter();
...
// wallpapers array now lives in js/config.js
...
// fileSystemData now lives in js/config.js
...
if (newBtn) {
  newBtn.addEventListener("click", () => {
    showToast("New Folder/File functionality coming soon");
  });
}
...
if (viewBtn) {
  viewBtn.addEventListener("click", () => {
    showToast("View options coming soon");
  });
}
...
startMenuIcons.forEach(icon => {
  icon.addEventListener("click", () => {
    ...
    if (typeof taskbarSearchInput !== "undefined" && taskbarSearchInput) taskbarSearchInput.value = "";
    if (typeof startSearchInput !== "undefined" && startSearchInput) startSearchInput.value = "";
    if (typeof resetAppFilter === "function") resetAppFilter();
  });
});
...
if (linkedInConnectBtn) {
  linkedInConnectBtn.addEventListener("click", () => {
    showToast("Connection request sent! Andrew will review your request soon.");
  });
}

if (linkedInMessageBtn) {
  linkedInMessageBtn.addEventListener("click", () => {
    showToast("📧 Send a message to andrewafdo@gmail.com")
  });
}
...
// gameIcons now lives in js/config.js
...
// =============================
// =============================
// ALT+Q WINDOW SWITCHER (Windows Alt-Tab style overlay)
// =============================

const switcherOverlay = document.getElementById("window-switcher");
const switcherCards = document.getElementById("window-switcher-cards");
...
function getWindowTitle(win) {
  ...
  return span ? span.textContent.trim() : win.id;
}

function getWindowIconSrc(win) {
  const iconBtn = document.querySelector(`[data-window="${win.id}"]`);
...
function renderSwitcher() {
  ...
  switcherCards.innerHTML = "";

  switcherOrder.forEach((win, i) => {

    const card = document.createElement("div");
    card.className = "switcher-card" + (i === switcherIndex ? " selected" : "");

    const icon = document.createElement("img");
    icon.src = getWindowIconSrc(win);
    ...
    switcherCards.appendChild(card);
...
function advanceSwitcher() {
  switcherIndex = (switcherIndex + 1) % switcherOrder.length;
  renderSwitcher();
}

function closeSwitcher(commit) {

  if (commit && switcherOrder[switcherIndex]) {
    focusWindow(switcherOrder[switcherIndex]);
  }
...
document.addEventListener("keyup", (e) => {
  ...
  if (e.key === "Alt" && switcherActive) {
    closeSwitcher(true);
  }

});
// Returns the currently-focused (highest z-index, visible) window, if any.
...
document.addEventListener("keydown", (e) => {
  ...
  // Don't hijack Esc/Alt+Tab while the user is typing in an input/textarea,
  // EXCEPT Esc, which should still be able to close things like the terminal.
